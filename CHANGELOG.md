...
- `article-interactive.css`：标题锚点图标放大为 `clamp(1.125rem, 0.65em, 1.5rem)`，并加宽左侧 gutter 以免与标题文字重叠。
...
- 浅色主题次要实心按钮：`--site-button-bg` 由 `gray-200` 改为 `gray-100`；悬停/按下依次为 `gray-200` / `gray-300`。

## 0.14.0 - 2026-06-26

### Changed

- `article.css`：正文与标题、行内代码、代码块字号全部由 `em` 改为 `rem`（基准 `1rem` = 16px），不再随 `--site-content-font-size` 缩放；间距仍保留与元素字号成比例的 `em`（如标题 `margin-top`）。
